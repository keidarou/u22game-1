...
public class movetheball : MonoBehaviour
{
    ...
    int karix, kariy;
    Vector3 upvectormokuteki, upvectornow, downvectornow, downvectormokuteki=Vector3.zero;//移動にはvectorにする必要がある
    //-------------------------------------------------
    ...
    bool ballmove()//玉を動かすぞい
    {
        ...
        downvectormokuteki += new Vector3(movexhoukou * haba * down, moveyhoukou * haba * down, -1.00f);//目的なので、それに方向×距離を足す
        upvectormokuteki = ballup.transform.position;//同様
        upvectormokuteki -= new Vector3( movexhoukou * haba * up,  moveyhoukou * haba * up, 0.00f);//同様
    ...
    void Update()
    {
        ...
        Debug.Log(upvectornow.x);
        Debug.Log(upvectornow.y);
